...
def plot_instance_summary(
    instances, group_cols, color_map=plt.cm.Set3, label_fontsize=9
):
    """
    ...
    if not group_cols:
        print("Warning: No group columns specified.")
        return
    ...
    for i, inst in enumerate(instances):
        for col in attr_names:
            try:
                value = getattr(inst, col, None)
                # Handle None values to avoid DataFrame creation issues
                if value is None:
                    value = "N/A"
                attr_names[col].append(value)
            except Exception as e:
                print(f"Error accessing attribute '{col}' from instance {i}: {e}")
                attr_names[col].append("N/A")

    # Check if all lists have the same length
    lengths = [len(attr_names[col]) for col in attr_names]
    if len(set(lengths)) > 1:
        print(
            f"Warning: Inconsistent list lengths: {dict(zip(attr_names.keys(), lengths))}"
        )

    # Ensure all lists have the same length by padding with None if necessary
    max_length = max(lengths) if lengths else 0
    for col in attr_names:
        while len(attr_names[col]) < max_length:
            attr_names[col].append("N/A")

    # Debug: Print data types and sample values
    print("Debug - attr_names structure:")
    for col, values in attr_names.items():
        print(
            f"  {col}: length={len(values)}, types={set(type(v).__name__ for v in values[:3])}, sample={values[:3]}"
        )
    ...
    try:
        ...
        print("Attempting to create DataFrame with string conversion...")
        # Convert all values to strings as a fallback
        attr_names_str = {
            col: [str(v) for v in values] for col, values in attr_names.items()
        }
        try:
            df_summary = pd.DataFrame(attr_names_str)
        except Exception as e2:
            print(f"Error creating DataFrame with strings: {e2}")
            # Final fallback: create empty DataFrame
            df_summary = pd.DataFrame(columns=list(attr_names.keys()))
    ...
    ax.set_xlabel(
        f"Combinations of ({', '.join(group_cols)})", fontsize=12, fontweight="bold"
    )
...
def explore_instance(data, model_type):
    ...
    if model_type.upper() == "MMNL":
        ...
        print(
            f"1. Utility Matrix (u): shape {u.shape}, range: [{u.flatten().min():.4f}, {u.flatten().max():.4f}]"
        )
        print(
            f"2. Product Prices (price): shape {data.price.shape},range: [{price.min():.4f}, {price.max():.4f}]"
        )
        print(f"3. No-Purchase Utility value (v0): {data.v0}")
        print(
            f"4. Customer Segment Probabilities (omega): shape {data.omega.shape}, sum: {omega.sum():.4f} (should be 1.0)"
        )
        ...
        print(
            f"1. Utility Matrix (v): shape {u.shape}, range: [{u.flatten().min():.4f}, {u.flatten().max():.4f}]"
        )
        print(
            f"2. Product Prices (price): shape {data.price.shape},range: [{price.min():.4f}, {price.max():.4f}]"
        )
        print(
            f"3. Dissimilarity Parameter (gamma): shape {data.gamma.shape},range: [{gamma.min():.4f}, {gamma.max():.4f}]"
        )
        print(f"4. No-Purchase Utility (v0): {v0:.4f}")
        if isinstance(vi0, np.ndarray) and vi0.size > 1:
            print(
                f"5. Within-Nest No-Purchase Utility (vi0): shape: {vi0.shape}, range: [{vi0_flat.min():.4f}, {vi0_flat.max():.4f}]"
            )
        else:
            print(
                f"5. Within-Nest No-Purchase Utility (vi0): {vi0:.4f} (uniform across nests)"
            )
    ...
    if model_type.upper() == "MMNL":
        ...
        ax4.pie(
            omega,
            labels=[f"Segment {i+1}" for i in range(len(omega))],
            autopct="%1.1f%%",
            colors=colors,
            startangle=90,
        )
        ...
        if isinstance(data.vi0, np.ndarray) and data.vi0.size > 1:
            ...
            ax4.pie(
                vi0,
                labels=[f"Nest {i+1}" for i in range(data.m)],
    ...
    ax5.set_title(
        (
            "Utility Statistics by Segment"
            if model_type.upper() == "MMNL"
            else "Utility Statistics by Nest"
        ),
        fontweight="bold",
    )
    ...
    ax6.set_xlabel(
        "Price" if model_type.upper() == "MMNL" else "Average Price Across Nests"
    )
    ax6.set_ylabel(
        "Average Utility Across Segments"
        if model_type.upper() == "MMNL"
        else "Average Utility Across Nests"
    )
...
def export_summary_statistics(
    ...
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        ...
        for method_value in df_results[method_col].unique():
            ...
            summary_detail = df_subset.groupby(detail_group_col)[
                "gap_method"
            ].describe()
        ...
        pivot_compare = df_results.pivot_table(
            values="gap_method", index=group_cols, columns=method_col, aggfunc="mean"
        )
...
def plot_comparison_boxplots(df, method_col, group_col):
    ...
    plt.suptitle(
        f"Algorithm Performance Comparison Across {method_label}",
        fontsize=14,
        fontweight="bold",
        y=1.02,
    )
    ...
    for method in methods:
        ...
        print(
            df_method.groupby(group_col)["gap_method"]
